...
* 编译是指把源文件编译称机器语言文件，还不是程序，只是一个机器语言文件。
* 因为即使最简单的helloworld都得调用很多库，所以往往只是这一个机器语言文件并不能构成一个程序。
* 所以有了build一词，代表构建一个可执行程序，其中包括编译各种源文件，链接各种库，最终生成或者说构建成或者说build了一个可执行程序。
...
* shell如此寻找程序的位置
  ...
  * 没有加路径，那么搜索PATH环境变量里面的所有目录。
    * 如果没找到，提示找不到，无法执行。
    * 找PATH环境变量里面的目录，不会递归查找，只会找一级，例如PATH里面一般有~/bin, 假设脚本xrayBoot，放在~/bin可以找到，放在~/bin/xray下就会找不到。
* 注
