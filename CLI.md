...
>
> 不同bash之间语言语法有些行为完全不同，这一点一定要注意，所以shebang很重要
...
* 使用
  ...
  * 使用echo结果
    ...
    * shell函数的返回值跟常规意义上的返回值不同，我们不能直接使用。它只能表示函数的执行成功与否，就这一个作用，不能用来与外界通信，想要通信的话用echo 输出，然后$()使用输出，或者使用全局变量
      * 如果命令包含有多个标准输出，那么$()只能捕获第一个。
      * 如果是自己写的脚本，经常有很多命令自带输出，然后影响我们真真想要的输出
      * 例如脚本中有git fetch,  有我们想要的echo， 如果git  fetch在前，那么$()会捕获其内容，而不是echo的内容，这时可以重定向我们不想要的输出到/dev/null, 
        * git fetch > /dev/null 问题解决，$()就能捕获我们自己的echo的输出了
  * 使用
